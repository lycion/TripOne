...
There are two ways to setup Xpedited connections.  First, you can use the `expedited` command in the tripone-cli or RPC calls to
...
The recommended method is to add entries in your tripone.conf file.  With this mechanism the configuration will persist and Xpedited service
will be restarted when either your node or the other is restarted.


2a. Setting up an Xpedited connection using tripone.conf
--------------------------------------------------------

Two tripone.conf entries need to be made.  One is the `addnode` connection which opens the connection to the remote peer and the 
...
For example, to turn on expedited forwarding of blocks from a remote peer to my peer you would enter the following:
	...
	tripone-cli expedited block "192.168.0.6:9773" on

and to do the same for transaction fowarding I would enter:

	tripone-cli expedited tx "192.168.0.6:9773" on


To get more help on how to use the RPC commands you can type in "tripone-cli help expedited" from the command line, or if you ar using the debug console
