...
- To auto-format a file:
  ...
  ========================
  Add the following to your $HOME/.gitconfig or .git/config in the Tripone directory:
  ```
[filter "tripone-clang-format"]
...
```
   Do not fix other people's formatting issues in your PR, since it distracts from the content!  Let the TripOneJanitor clean up other people's mess.
...
Run with the -testnet option to run with "play tripones" on the test network, if you
...
Tripone Unlimited is a multithreaded application, and deadlocks or other multithreading bugs
...
Tripone Unlimited can be compiled with the litonemalloc allocation library and
memory profiling tool.  First install litonemalloc either from source here
...
- ThreadSocketHandler : Sends/Receives data from peers on port 9773.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 9772 for connections and services them.

- TriponeMiner : Generates tripones (if wallet is enabled).
...
However, in open source software such as Tripone Unlimited, where everyone uses
...
pay attention to for reviewers of Tripone Unlimited code.

General Tripone Unlimited
----------------------
...
- For `strprintf`, `LOG`, `LOGA` formatting characters don't need size specifiers
  ...
  - *Rationale*: Tripone Unlimited uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of Tripone and Tripone Cash, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against Tripone Unlimited so that they can be integrated
...
- src/libsecp256k1
  - Upstream at https://github.com/tripone-core/secp256k1/ ; actively maintaned by Core contributors.

- src/crypto/ctaes
  - Upstream at https://github.com/tripone-core/ctaes ; actively maintained by Core contributors.
...
file descriptors it will cause problems with Tripone's `select()` loop, because
...
compatibility between nodes. This happened in Tripone 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Tripone's
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:tripone/tripone.git
...
Commit [`bb81e173`](https://github.com/tripone/tripone/commit/bb81e173) is an example of a scripted-diff.
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `tripone-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `tripone-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Tripone, the wallet was always
