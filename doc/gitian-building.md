...
*Setup instructions for a Gitian build of Tripone using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Tripone
...
- [Building Tripone](#building-tripone)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-tripone-host/README.md).
...
Clone the git repositories for tripone and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/TripOne/TripOne
```
...
in the tripone repository under 'Fetch and build inputs' to install sources which require
...
To build Tripone (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the tripone repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/tripone/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/TripOne/TripOne
...
For example:
```bash
URL=https://github.com/laanwj/tripone.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit tripone=${COMMIT} --url tripone=${URL} ../tripone/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit tripone=${COMMIT} --url tripone=${URL} ../tripone/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit tripone=${COMMIT} --url tripone=${URL} ../tripone/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the tripone git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../tripone/contrib/gitian-descriptors/*|sort|uniq )
...
TONEPATH=/some/root/path/tripone.git
SIGPATH=/some/root/path/tripone-detached-sigs.git

./bin/gbuild --url tripone=${TONEPATH},signature=${SIGPATH} ../tripone/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/tripone-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/tripone-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/tripone-osx-build.assert
```
...
[tripone/gitian.sigs](https://github.com/tripone/gitian.sigs/) repository, or if that's not possible create a pull
