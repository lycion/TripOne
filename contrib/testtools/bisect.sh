...
# based on https://github.com/ftrader/tripone-misc-contrib/blob/master/bisection-tools/bisect_probabilistic_with_timeout.sh
...
#
# 1. CAUTION!!!! This kills any running triponed's as part of cleanup!
...
# optional tool for use with "test_tripone" tests
...
# sequentially.
PARALLEL_TEST_TOOL=gtest-parallel-tripone
...
TEST_COMMAND=${3:-"src/test/test_tripone"}
...
# helper function to check if parameter is some form of test_tripone
# this function has been dumbed down for now.
is_test_tripone()
{
    if [ "$1" = "src/test/test_tripone" ]
    then
        return 0
    else
        return 1
...
do
    ...
    # check if the test is src/test/test_tripone
    is_test_tripone "${TEST_COMMAND}"
    if [ $? -eq 0 ]
    then
