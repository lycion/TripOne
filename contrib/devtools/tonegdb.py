...
class SpHelp(gdb.Command):
    def __init__(self,cmds):
        super(SpHelp, self).__init__("tone-help", gdb.COMMAND_DATA, gdb.COMPLETE_SYMBOL)
        self.cmds = cmds

    def help(self):
      return "tone-help: This help"
...
class BuDumpCBlockIndex(gdb.Command):
    def __init__(self):
        super(BuDumpCBlockIndex, self).__init__("tone-dump-bidx", gdb.COMMAND_DATA, gdb.COMPLETE_SYMBOL)

    def help(self):
      return "tone-dump-bidx CBlockIndex* count: Dump the CBlockIndex chain count elements deep"
    def invoke(self, argument, from_tty):
        args = gdb.string_to_argv(argument)
        if len(args)!=2:
          gdb.write("args:\n  CBlockIndex*: pointer to the chain\n  number: how far to follow the chain\nexample: tone-dump-bidx pindex 10\n")
          return
...
gdb.write("Loaded Tripone Unlimited GDB extensions %s.\nRun 'tone-help' for command help\n" % VERSION)
