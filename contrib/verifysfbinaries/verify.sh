...
###   This script attempts to download the signature file SHA256SUMS.asc from tripone.org
...
WORKINGDIR="/tmp/tripone"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://tripone.org/bin/"
VERSIONPREFIX="tripone-unlimited-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'tripone-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Tripone binary release signing key installed?"
   fi
