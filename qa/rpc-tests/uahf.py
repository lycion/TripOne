...
invalidOpReturn = hexlify(b'Tripone: A Peer-to-Peer Electronic Cash System')

class BUIP055Test (TriponeTestFramework):
    def __init__(self, extended=False):
        self.extended = extended
        TriponeTestFramework.__init__(self)
    ...
    def testDefaults(self):
        for n in self.nodes:
            ...
            if int(nodeInfo["localservices"],16)&NODE_TRIPONE_CASH:
                assert(t['mining.forkTime'] == 1501590000)  # Tripone Cash release REQ-2
            else:
    ...
    def testCli(self):
        ...
        # if this is a tripone cash build, we need to do the cash defaults on our old chain node
        if int(nodeInfo["localservices"],16)&NODE_TRIPONE_CASH:
    ...
    def testNetMagic(self):
        ...
        if int(info["localservices"],16)&NODE_TRIPONE_CASH:
            try: # Accept BU cash nodes if running TONE node
                buCashNode.cnxns[0].wait_for_buverack()
            except DisconnectedError:
                assert(not "should not have disconnected a tripone cash node")
        else:
            try: # do not accept BU cash nodes if running TONE node
                buCashNode.cnxns[0].wait_for_buverack()
                assert(not "should have disconnected a tripone cash node")
            except DisconnectedError:
