...
def GenerateSingleSigP2SH(toneAddress):
    redeemScript = CScript([OP_DUP, OP_HASH160, triponeAddress2bin(toneAddress), OP_EQUALVERIFY, OP_CHECKSIG])
    p2shAddressBin = hash160(redeemScript)
    p2shAddress = encodeTriponeAddress(bytes([196]), p2shAddressBin)  # 196 is regtest P2SH addr prefix
...
class WalletTest (TriponeTestFramework):
    ...
    def check_fee_amount(self, curr_balance, balance_with_fee, fee_per_byte, tx_size):
        ...
        if fee < target_fee:
            raise AssertionError("Fee of %s TONE too low! (Should be %s TONE)"%(str(fee), str(target_fee)))
        # allow the node's estimation to be at most 2 bytes off
        if fee > fee_per_byte * (tx_size + 2):
            raise AssertionError("Fee of %s TONE too high! (Should be %s TONE)"%(str(fee), str(target_fee)))
        return curr_balance

    def setup_chain(self,triponeConfDict=None, wallets=None):
        logging.info("Initializing test directory "+self.options.tmpdir)
        initialize_chain_clean(self.options.tmpdir, 4, triponeConfDict, wallets)
    ...
    def run_test (self):
        ...
        # Send 21 TONE from 0 to 2 using sendtoaddress call.
        ...
        # node0 should end up with 100 tone in block rewards plus fees, but
        ...
        # Send 10 TONE with subtract fee from amount
        ...
        # Sendmany 10 TONE with subtract fee from amountd
        ...
        # now try P2SH
        toneAddress = self.nodes[1].getnewaddress()
        toneAddress = self.nodes[1].getaddressforms(toneAddress)["legacy"]
        ( p2shAddress, redeemScript) = GenerateSingleSigP2SH(toneAddress)
        self.nodes[0].sendtoaddress(p2shAddress,1)

        toneAddress2 = self.nodes[1].getnewaddress()
        toneAddress2 = self.nodes[1].getaddressforms(toneAddress2)["legacy"]
        ( p2shAddress2, redeemScript2) = GenerateSingleSigP2SH(toneAddress2)
        ...
        txns = self.nodes[2].listreceivedbyaddress(0, True, True)
        assert_array_result(txns,
                            {"address": self.nodes[2].getaddressforms(p2shAddress)["triponecash"]},
                            {"label": ""})
        assert_array_result(txns,
                            {"address": self.nodes[2].getaddressforms(p2shAddress2)["triponecash"]},
                            {"label": ""})
...
def Test():
    ...
    t.main(["--nocleanup", "--noshutdown"], triponeConf, None)
