...
def triponeAddress2bin(toneAddress):
    """convert a tripone address to binary data capable of being put in a CScript"""
    # chop the version and checksum out of the bytes of the address
    return decodeBase58(toneAddress)[1:-4]
...
def createWastefulOutput(toneAddress):
    """ Warning: Creates outputs that can't be spent by triponed"""
...
this is junk data. this is junk data. this is junk data. this is junk data. this is junk data."""
    ret = CScript([data, OP_DROP, OP_DUP, OP_HASH160, triponeAddress2bin(toneAddress), OP_EQUALVERIFY, OP_CHECKSIG])
    return ret


def p2pkh(toneAddress):
    """ create a pay-to-public-key-hash script"""
    ret = CScript([OP_DUP, OP_HASH160, triponeAddress2bin(toneAddress), OP_EQUALVERIFY, OP_CHECKSIG])
    return ret


def createrawtransaction(inputs, outputs, outScriptGenerator=p2pkh):
    """
    Create a transaction with the exact input and output syntax as the tripone-cli "createrawtransaction" command.
    If you use the default outScriptGenerator, this function will return a hex string that exactly matches the
    output of tripone-cli createrawtransaction.

    But this function is extended beyond tripone-cli in the following ways:
    ...
    for addr, amount in pairs:
        if callable(addr):
            tx.vout.append(CTxOut(amount * TONE, addr()))
        elif type(addr) is list:
            tx.vout.append(CTxOut(amount * TONE, CScript(addr)))
        elif addr == "data":
            tx.vout.append(CTxOut(0, CScript([OP_RETURN, unhexlify(amount)])))
        else:
            tx.vout.append(CTxOut(amount * TONE, outScriptGenerator(addr)))
    tx.rehash()
