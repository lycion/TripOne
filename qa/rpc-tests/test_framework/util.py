...
#
import pdb
# Add python-triponerpc to module search path:
import os
import sys
import math
import binascii
...
class NoConfigValue:
    """ Use to remove the specific configure parameter and value when writing to tripone.conf"""
    def __init__(self):
        pass
...
def enable_mocktime():
    # Set the mocktime to be after the Tripone Cash fork so
...
def check_json_precision():
    """Make sure json library being used does not lose precision converting TONE values"""
...
def initialize_datadir(dirname, n, triponeConfDict=None, wallet=None, bins=None):
    ...
    if triponeConfDict: defaults.update(triponeConfDict)
...
def wait_for_triponed_start(process, url, i):
    '''
    Wait for triponed to start. This means that RPC is accessible and fully initialized.
    Raise an exception if triponed exits during initialization.
    '''
    while True:
        if process.poll() is not None:
            raise Exception('triponed exited with status %i during initialization' % process.returncode)
        try:
...
def initialize_chain(test_dir,triponeConfDict=None,wallets=None, bins=None):
    """
    ...
    if (not os.path.isdir(os.path.join("cache","node0"))
        ...
        # Create cache directories, run triponeds:
        for i in range(4):
            for retry in range(4):
                try:
                    datadir=initialize_datadir("cache", i, triponeConfDict, wallets, bins)
                    if bins:
                        args = [ bins[i], "-keypool=1", "-datadir="+datadir ]
                    else:
                        args = [ os.getenv("TRIPONED", "triponed"), "-keypool=1", "-datadir="+datadir ]
                    if i > 0:
                        args.append("-connect=127.0.0.1:"+str(p2p_port(0)))
                    triponed_processes[i] = subprocess.Popen(args)
                    if os.getenv("PYTHON_DEBUG", ""):
                        print("initialize_chain: triponed started, waiting for RPC to come up")
                    wait_for_triponed_start(triponed_processes[i], rpc_url(i), i)
                    if os.getenv("PYTHON_DEBUG", ""):
                        print("initialize_chain: RPC succesfully started")
                    break
                except Exception as exc:
                    logging.error("Error bringing up triponed #%d (initialize_chain, directory %s), this might be retried. Problem is:", i, test_dir)
                    do_and_ignore_failure(triponed_processes[i].kill)
                    traceback.print_exc(file=sys.stdout)
                    remap_ports(i)
            else:
                raise Exception("Couldn't start triponed even with retries on different ports (initialize_chain).")
    ...
    for i in range(4):
        ...
        initialize_datadir(test_dir, i,triponeConfDict,wallets[i] if wallets else None) # Overwrite port/rpcport in tripone.conf

def initialize_chain_clean(test_dir, num_nodes, triponeConfDict=None, wallets=None):
    """
    Create an empty blockchain and num_nodes wallets.
    Useful if a test case wants complete control over initialization.
    """
    for i in range(num_nodes):
        datadir=initialize_datadir(test_dir, i, triponeConfDict, wallets)
...
def start_node(i, dirname, extra_args=None, rpchost=None, timewait=None, binary=None):
    """
    Start a triponed and return RPC connection to it
    """
    datadir = os.path.join(dirname, "node"+str(i))
    if binary is None:
        binary = os.getenv("TRIPONED", "triponed")
    ...
    for retry in range(4):
        try:
            triponed_processes[i] = subprocess.Popen(args)
            if os.getenv("PYTHON_DEBUG", ""):
                print("start_node: triponed started, waiting for RPC to come up")
            url = rpc_url(i, rpchost)
            wait_for_triponed_start(triponed_processes[i], url, i)
            if os.getenv("PYTHON_DEBUG", ""):
                print("start_node: RPC succesfully started")
            break
        except Exception as exc:
            logging.error("Error bringing up triponed #%d (start_node, directory %s), this might be retried. Problem is:", i, dirname)
            do_and_ignore_failure(triponed_processes[i].kill)
            traceback.print_exc(file=sys.stdout)
            remap_ports(i)
            fixup_ports_in_configfile(i)
    else:
        raise Exception("Couldn't start triponed even with retries on different ports (start_node).")
...
def start_nodes(num_nodes, dirname, extra_args=None, rpchost=None, binary=None,timewait=None):
    """
    Start multiple triponeds, return RPC connections to them
    """
...
def stop_node(node, i):
    ...
    triponed_processes[i].wait(timeout=TRIPONED_PROC_WAIT_TIMEOUT)
    del triponed_processes[i]
...
def wait_triponeds():
    # Wait for all triponeds to cleanly exit
    for triponed in triponed_processes.values():
        triponed.wait(timeout=TRIPONED_PROC_WAIT_TIMEOUT)
    triponed_processes.clear()
...
def split_transaction(node, prevouts, toAddrs, txfeePer=DEFAULT_TX_FEE_PER_BYTE,**kwargs):
    ...
    try:  # try finally block to put the decimal precision back to what it was prior to this routine
      decimal.getcontext().prec = 8 + 8 # 8 digits to get to 21million, and each tripone is 100 million satoshis
      amount = Decimal(0)
      iamount = 0
      count = 0
      for tx in prevouts:
        inp.append({"txid":str(tx["txid"]),"vout":tx["vout"]})
        amount += tx["amount"]*Decimal(TONE)
        iamount += int(tx["amount"]*Decimal(TONE))
        count += 1
      ...
      while 1:
          ...
          for a in toAddrs[0:-1]:
            if PerfectFractions:
              outp[str(a)] = str(amtPer/Decimal(TONE))
            else:
              outp[str(a)] = float(amtPer/TONE)

          a = toAddrs[-1]
          amtPer = (amount - ((len(toAddrs)-1)*amtPer)) - txfee
          # print "final amt: ", amtPer
          if PerfectFractions:
              outp[str(a)] = str(amtPer/TONE)
          else:
              outp[str(a)] = float(amtPer/TONE)
