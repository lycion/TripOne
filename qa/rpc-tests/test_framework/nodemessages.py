...
COIN = 100000000  # 1 tone in satoshis
...
def encodeTriponeAddress(prefix, data):
    data2 = prefix + data
...
def hash256(s):
    """Return the double SHA256 hash (what tripone typically uses) of the passed binary data
...
def deser_string(f):
    """Convert an array of bytes in the tripone P2P protocol format into a string
...
def ser_string(s):
    """convert a string into an array of bytes (in the tripone network format)
...
# Objects that map to triponed objects, which can be serialized/deserialized
...
class msg_headers(object):
    ...
    def deserialize(self, f):
        # comment in triponed indicates these should be deserialized as blocks
