...
def init(libtriponecashfile=None):
    global cashlib
    if libtriponecashfile is None:
        libtriponecashfile = "libtriponecash.so"
        try:
            cashlib = CDLL(libtriponecashfile)
        except OSError:
            import os
            dir_path = os.path.dirname(os.path.realpath(__file__))
            cashlib = CDLL(dir_path + os.sep + libtriponecashfile)
    else:
        cashlib = CDLL(libtriponecashfile)
    if cashlib is None:
        raise Error("Cannot find %s shared library", libtriponecashfile)
...
def signTxInput(tx, inputIdx, inputAmount, prevoutScript, key, sigHashType=SIGHASH_FORKID | SIGHASH_ALL):
    ...
    """
    assert (sigHashType & SIGHASH_FORKID) > 0, "Did you forget to indicate the tripone cash hashing algorithm?"
    if type(tx) == str:
        tx = unhexlify(tx)
    elif type(tx) != bytes:
        tx = tx.serialize()
