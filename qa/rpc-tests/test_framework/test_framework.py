...
class TriponeTestFramework(object):
    ...
    def setup_chain(self,triponeConfDict=None, wallets=None):
        """
        Sets up the blockchain for the tripone nodes.  It also sets up the daemon configuration.
        triponeConfDict:  Pass a dictionary of values you want written to tripone.conf.  If you have a key with multiple values, pass a list of the values as the value, for example:
        ...
        """
        print("Initializing test directory ", self.options.tmpdir, "Tripone conf: ", str(triponeConfDict), "walletfiles: ", wallets)
        initialize_chain(self.options.tmpdir,triponeConfDict, wallets, self.bins)
    ...
    def main(self,argsOverride=None,triponeConfDict=None,wallets=None):
        """
        ...
        parser.add_option("--nocleanup", dest="nocleanup", default=False, action="store_true",
                          help="Leave triponeds and test.* datadir on exit or error")
        parser.add_option("--noshutdown", dest="noshutdown", default=False, action="store_true",
                          help="Don't stop triponeds after the test execution")
        parser.add_option("--srcdir", dest="srcdir", default=os.path.normpath(os.path.dirname(os.path.realpath(__file__))+"/../../../src"),
                          help="Source directory containing triponed/tripone-cli (default: %default)")
        ...
        try:
            ...
            if self.setup_chain.__defaults__ is None:
              ...
              self.setup_chain(triponeConfDict, wallets)
        ...
        if not self.options.noshutdown:
            ...
            print("Note: triponeds were not stopped and may still be running")
...
# Test framework for doing p2p comparison testing, which sets up some triponed
# binaries:
# 1 binary: test binary
# 2 binaries: 1 test binary, 1 ref binary
# n>2 binaries: 1 test binary, n-1 ref binaries

class ComparisonTestFramework(TriponeTestFramework):
    ...
    def add_options(self, parser):
        parser.add_option("--testbinary", dest="testbinary",
                          default=os.getenv("TRIPONED", "triponed"),
                          help="triponed binary to test")
        parser.add_option("--refbinary", dest="refbinary",
                          default=os.getenv("TRIPONED", "triponed"),
                          help="triponed binary to use for reference nodes (if any)")

    def setup_chain(self,triponeConfDict=None, wallets=None):  # BU add config params
        print("Initializing test directory ", self.options.tmpdir)
        initialize_chain_clean(self.options.tmpdir, self.num_nodes,triponeConfDict, wallets)
