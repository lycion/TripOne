...
#
# mininode.py - Tripone P2P network half-a-node
#
# This python code was modified from ArtForz' public domain  half-a-node, as
# found in the mini-node branch of http://github.com/jgarzik/pynode.
#
# NodeConn: an object which manages p2p connectivity to a tripone node
...
class NodeConn(asyncore.dispatcher):
    ...
    def __init__(self, dstaddr, dstport, rpc, callback, net="regtest", services=1, triponeCash=True):
        self.triponeCash = triponeCash
        if self.triponeCash:
            self.MAGIC_BYTES = self.CASH_MAGIC_BYTES
        else:
            self.MAGIC_BYTES = self.TONE_MAGIC_BYTES
        ...
        print('MiniNode: Connecting to Tripone Node IP # ' + dstaddr + ':'
              + str(dstport))
        try:
    ...
    def got_data(self):
        ...
        try:
            while True:
                ...
                if nowLen < 4:
                    return
                if (self.recvbuf[:4] != self.MAGIC_BYTES[self.network]) and (self.recvbuf[:4] != self.TONE_MAGIC_BYTES[self.network]):
