...
class TransactionPerformanceTest(TriponeTestFramework):

    def setup_chain(self,triponeConfDict=None, wallets=None):
        logging.info("Initializing test directory "+self.options.tmpdir)
        initialize_chain_clean(self.options.tmpdir, 3, triponeConfDict, wallets)

    def setup_network(self, split=False):
        self.nodes = start_nodes(3, self.options.tmpdir,timewait=60*60)

        #connect to a local machine for debugging
        #url = "http://triponerpc:DP6DvqZtqXarpeNWyN3LZTFchCCyCUuHwNF7E8pX99x1@%s:%d" % ('127.0.0.1', 19772)
...
if __name__ == '__main__':
    ...
    tpt.main(["--nocleanup"],triponeConf)
...
def Test():    
    ...
    tpt.main(["--nocleanup","--tmpdir=/ramdisk/test"],triponeConf)
