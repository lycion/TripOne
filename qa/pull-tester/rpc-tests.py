...
`qa/pull-tester/test_framework/test_framework.py:TriponeTestFramework.main`.
...
if 'ENABLE_WALLET' not in vars():
    ENABLE_WALLET=0
if 'ENABLE_TRIPONED' not in vars():
    ENABLE_TRIPONED=0
...
#Set env vars
if "TRIPONED" not in os.environ:
    os.environ["TRIPONED"] = BUILDDIR + '/src/triponed' + EXEEXT
if "TRIPONECLI" not in os.environ:
    os.environ["TRIPONECLI"] = BUILDDIR + '/src/tripone-cli' + EXEEXT
...
if not (ENABLE_WALLET == 1 and ENABLE_UTILS == 1 and ENABLE_TRIPONED == 1):
    print("No rpc tests to run. Wallet, utils, and triponed must all be enabled")
    sys.exit(0)
...
def runtests():
    ...
    if(ENABLE_WALLET == 1 and ENABLE_UTILS == 1 and ENABLE_TRIPONED == 1):
...
class RPCTestHandler:
    ...
    def __init__(self, num_tests_parallel, test_list=None, flags=None):
        ...
        self.flags = flags
        self.num_running = 0
        # In case there is a graveyard of zombie triponeds, we can apply a
        ...
        while True:
            ...
            for j in self.jobs:
                ...
                if proc.poll() is not None:
                    ...
                    stderr_filtered = re.sub(r"Error: Unable to bind to 0.0.0.0:[0-9]+ on this computer\. Tripone Unlimited Cash Edition is probably already running\.",
                                             "", stderr_filtered)
...
class RPCCoverage(object):
    ...
    commands per `tripone-cli help` (`rpc_interface.txt`).
