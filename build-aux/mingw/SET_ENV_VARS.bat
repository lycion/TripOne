...
REM          of your tripone source under ".\build-aux\mingw\".  If this is true, you do not need
REM          to modify the TRIPONE_GIT_ROOT path.  Otherwise update this with the absolute path to
...
REM The latest versions of the tripone client uses Python3, while previous versions use Python2
...
REM Set the dependency path.  This is where all of the Tripone dependencies will be downloaded and built
...
REM Set the path to your Tripone git checkout.
REM NOTE: If you are running these scripts from the tripone checkout location, you do not need to modify this
set "TRIPONE_GIT_ROOT=%CD%\..\..\"


REM ##################################################################################################
REM These parameters configure how the tripone client and dependencies are built.
REM
...
REM    6. Strip enabled
REM    7. Skip Tripone autogen.sh step disabled (useful for rebuilding local changes only)
REM    8. Skip Tripone configure step disbled (useful for rebuilding local changes only)
REM
...
REM Following will strip debug symbols from the generated tripone executables, greatly reducing file size.
REM This will, however, make it more difficult to debug any issues that may occur while testing.
REM If you want to keep debug symbols in the generated tripone executables, uncomment the line below.
SET STRIP=YES

REM If you want to skip running "./autogen.sh" when building tripone uncomment the line below.
...
REM NOTE: This only affects the build of the tripone executables, not any of the dependencies.
...
REM If you want to skip running "./configure" when building tripone uncomment the line below.
REM This is useful to turn off if you have already run this once and not changed the toolchain
