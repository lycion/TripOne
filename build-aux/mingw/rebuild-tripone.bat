...
if "%BUILD_ARCH%" NEQ "T" (
	...
	echo of the Tripone client in SET_ENV_VARS.bat.
...
if not exist "%MINGW_ROOT%" (
	...
	echo Aborting build of Tripone client...
	pause
	exit /b -1
)
...
if not exist "%MSYS_SH%" (
	echo MSYS does not appear to have installed correctly.  Build of Tripone client
...
if "%BUILD_32_BIT%" NEQ "" (
	...
	set "BUILD_OUTPUT=%TRIPONE_GIT_ROOT%\build-output\x86"
...
if "%BUILD_64_BIT%" NEQ "" (
	...
	set "BUILD_OUTPUT=%TRIPONE_GIT_ROOT%\build-output\x64"
...
%MSYS_SH% "%INST_DIR%\make-tripone.sh"
REM Check to see if make-tripone.sh failed (possibly due to missing dependencies)
if %errorlevel% neq 0 (
...
echo Saving tripone executables to %BUILD_OUTPUT%
REM Make sure output directory exists
mkdir "%BUILD_OUTPUT%\"

REM cd to src to copy tripone-tx.exe, tripone-cli.exe, and triponed.exe
cd "%TRIPONE_GIT_ROOT%\src"
copy tripone-tx.exe "%BUILD_OUTPUT%\tripone-tx.exe"
copy tripone-cli.exe "%BUILD_OUTPUT%\tripone-cli.exe"
copy triponed.exe "%BUILD_OUTPUT%\triponed.exe"
copy tripone-miner.exe "%BUILD_OUTPUT%\tripone-miner.exe"

REM cd to src\qt to copy tripone-qt.exe
cd qt
copy tripone-qt.exe "%BUILD_OUTPUT%\tripone-qt.exe"
