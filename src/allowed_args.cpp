...
// These globals are needed here so tripone-cli can link
const std::string CURRENCY_UNIT = "BCH";
const std::string DEFAULT_TOR_CONTROL = "127.0.0.1:9051";
const char DEFAULT_RPCCONNECT[] = "127.0.0.1";

// Variables for traffic shaping.  Needed here so tripone-cli can link
...
static void addConfigurationLocationOptions(AllowedArgs &allowedArgs)
{
    allowedArgs.addHeader(_("Configuration location options:"))
        .addArg(
            "conf=<file>", requiredStr, strprintf(_("Specify configuration file (default: %s)"), TRIPONE_CONF_FILENAME))
        .addArg(
...
#ifndef WIN32
        .addArg("pid=<file>", requiredStr, strprintf(_("Specify pid file (default: %s)"), TRIPONE_PID_FILENAME))
#endif
...
static void addWalletOptions(AllowedArgs &allowedArgs)
{
    ...
    allowedArgs.addHeader(_("Wallet options:"))
        ...
        .addArg("usecashaddr", optionalBool,
            _("Use Tripone Cash Address for destination encoding (Activates by default Jan 14, 2017)"));
#endif
}
...
static void addRpcServerOptions(AllowedArgs &allowedArgs)
{
    allowedArgs.addHeader(_("RPC server options:"))
        ...
        // Although a node does not use rpcconnect it must be allowed because TriponeCli also uses the same config file
...
// tripone-cli does not know about tweaks so we have to silently ignore unknown options
TriponeCli::TriponeCli() : AllowedArgs(true)
{
    addHelpOptions(*this);
...
Triponed::Triponed(CTweakMap *pTweaks) : AllowedArgs(false) { addAllNodeOptions(*this, HMM_TRIPONED, pTweaks); }
TriponeQt::TriponeQt(CTweakMap *pTweaks) : AllowedArgs(false) { addAllNodeOptions(*this, HMM_TRIPONE_QT, pTweaks); }
TriponeTx::TriponeTx() : AllowedArgs(false)
{
    addHelpOptions(*this);
    addChainSelectionOptions(*this);
    ...
    addHeader(_("Transaction options:"))
        ...
        .addDebugArg("", optionalBool, "Read hex-encoded tripone transaction from stdin.");
}

ConfigFile::ConfigFile(CTweakMap *pTweaks) : AllowedArgs(false)
{
    // Merges all allowed args from TriponeCli, Triponed, and TriponeQt.
    // Excludes args from TriponeTx, because tripone-tx does not read
    // from the config file. Does not set a help message, because the
    // program does not output a config file help message anywhere.

    TriponeCli triponeCli;
    Triponed triponed(pTweaks);
    TriponeQt triponeQt;

    m_args.insert(triponeCli.getArgs().begin(), triponeCli.getArgs().end());
    m_args.insert(triponed.getArgs().begin(), triponed.getArgs().end());
    m_args.insert(triponeQt.getArgs().begin(), triponeQt.getArgs().end());
}
