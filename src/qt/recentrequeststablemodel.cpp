    ...
    if (role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return TriponeUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount,
                    false, TriponeUnits::separatorNever);
            else
                return TriponeUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
bool RecentRequestsTableModel::setData(const QModelIndex &index, const QVariant &value, int role) { return true; }
...
QString RecentRequestsTableModel::getAmountTitle()
{
    ...
    {
        amountTitle += " (" + TriponeUnits::name(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")";
    }
    return amountTitle;
}
...
void RecentRequestsTableModel::updateDisplayUnit() { updateAmountColumnTitle(); }
