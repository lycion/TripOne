...
#include "triponeaddressvalidator.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Tripone address (e.g. %1)")
                                   .arg(QString::fromStdString(DummyAddress(params, GetConfig()))));
    widget->setValidator(new TriponeAddressEntryValidator(params.CashAddrPrefix(), parent));
    widget->setCheckValidator(new TriponeAddressCheckValidator(parent));
}
...
QString triponeURIScheme(const CChainParams &params, bool useCashAddr)
{
    if (!useCashAddr)
    {
        return "triponecash";
...
QString triponeURIScheme(const Config &cfg)
{
    return triponeURIScheme(cfg.GetChainParams(), cfg.UseCashAddrEncoding());
}
...
bool parseTriponeURI(const QString &scheme, const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if (!i->second.isEmpty())
            {
                if (!TriponeUnits::parse(TriponeUnits::BCH, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseTriponeURI(const QString &scheme, QString uri, SendCoinsRecipient *out)
{
    //
    //    Cannot handle this later, because triponecash://
    ...
    return parseTriponeURI(scheme, uriInstance, out);
}

QString formatTriponeURI(const Config &cfg, const SendCoinsRecipient &info)
{
    QString ret = info.address;
    if (!cfg.UseCashAddrEncoding())
    {
        // prefix address with uri scheme for base58 encoded addresses.
        ret = (triponeURIScheme(cfg) + ":%1").arg(ret);
    }
    int paramCount = 0;
    ...
    if (info.amount)
    {
        ret += QString("?amount=%1")
                   .arg(TriponeUnits::format(TriponeUnits::BCH, info.amount, false, TriponeUnits::separatorNever));
        paramCount++;
    }
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Tripone.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Tripone (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Tripone (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Tripone*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "tripone.desktop";
    return GetAutostartDir() / strprintf("tripone-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a tripone.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Tripone\n";
        else
            optionFile << strprintf("Name=Tripone (%s)\n", chain);
...
bool GetStartOnSystemStartup()
{
    CFURLRef triponeAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    LSSharedFileListRef loginItems = LSSharedFileListCreate(NULL, kLSSharedFileListSessionLoginItems, NULL);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, triponeAppUrl);
    // findStartupItemInList retains the item it returned, need to release
    if (foundItem)
        CFRelease(foundItem);
    CFRelease(loginItems);
    CFRelease(triponeAppUrl);
    return !!foundItem;
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    {
        // add tripone app to startup item list
        LSSharedFileListInsertItemURL(
            loginItems, kLSSharedFileListItemBeforeFirst, NULL, NULL, triponeAppUrl, NULL, NULL);
    }
...
QString uriPrefix() { return "triponecash"; }
} // namespace GUIUtil
