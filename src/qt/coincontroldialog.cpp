    ...
    : QDialog(parent), ui(new Ui::CoinControlDialog), model(0), platformStyle(_platformStyle)
{
    ...
    // see https://github.com/tripone/tripone/issues/5716
    ...
    // store amount int64 in this column, but don't show it
...
{
    GUIUtil::setClipboard(TriponeUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::clipboardQuantity() { GUIUtil::setClipboard(ui->labelCoinControlQuantity->text()); }
...
void CoinControlDialog::clipboardPriority() { GUIUtil::setClipboard(ui->labelCoinControlPriority->text()); }
// copy label "Dust" to clipboard
void CoinControlDialog::clipboardLowOutput() { GUIUtil::setClipboard(ui->labelCoinControlLowOutput->text()); }
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog *dialog)
{
    ...
    int nDisplayUnit = TriponeUnits::BCH;
    ...
    l2->setText(TriponeUnits::formatWithUnit(nDisplayUnit, nAmount)); // Amount
    l3->setText(TriponeUnits::formatWithUnit(nDisplayUnit, nPayFee)); // Fee
    l4->setText(TriponeUnits::formatWithUnit(nDisplayUnit, nAfterFee)); // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes)); // Bytes
    l6->setText(sPriorityLabel); // Priority
    l7->setText(fDust ? tr("yes") : tr("no")); // Dust
    l8->setText(TriponeUnits::formatWithUnit(nDisplayUnit, nChange)); // Change
    if (nPayFee > 0 && (coinControl->nMinimumTotalFee < nPayFee))
    {
    ...
    toolTip1 += tr("This means a fee of at least %1 per kB is required.")
                    .arg(TriponeUnits::formatHtmlWithUnit(nDisplayUnit, CWallet::GetRequiredFee(1000))) +
                "<br /><br />";
    ...
    QString toolTip3 = tr("This label turns red if any recipient receives an amount smaller than %1.")
                           .arg(TriponeUnits::formatHtmlWithUnit(nDisplayUnit, ::minRelayTxFee.GetFee(546)));
    ...
    for (const PAIRTYPE(QString, std::vector<COutput>) & coins : mapCoins)
    {
        ...
        for (const COutput &out : coins.second)
        {
            ...
            if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress))
            {
                ...
                // if listMode or change => show tripone address. In tree mode,
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, TriponeUnits::format(nDisplayUnit, out.tx->vout[out.i].nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, TriponeUnits::format(nDisplayUnit, nSum));
