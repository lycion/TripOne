...
const int TRIPONE_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
// BIP70 payment protocol messages
const char *BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char *BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char *BIP71_MIMETYPE_PAYMENT = "application/triponecash-payment";
const char *BIP71_MIMETYPE_PAYMENTACK = "application/triponecash-paymentack";
const char *BIP71_MIMETYPE_PAYMENTREQUEST = "application/triponecash-paymentrequest";
...
void PaymentServer::LoadRootCAs(X509_STORE *_store)
{
    ...
    if (certFile.isEmpty())
    {
        qDebug() << QString("PaymentServer::%1: Payment request authentication via X.509 certificates disabled.")
                        .arg(__func__);
        return;
    }
...
{
    const QString scheme = GUIUtil::triponeURIScheme(params, useCashAddr);
    ...
    if (!GUIUtil::parseTriponeURI(scheme, arg, &r))
    {
        return {};
    }
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    Q_FOREACH (const QString &r, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(TRIPONE_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = NULL;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(TRIPONE_IPC_CONNECT_TIMEOUT);
...
PaymentServer::PaymentServer(QObject *parent, bool startLocalServer)
    : QObject(parent), saveURIs(true), uriServer(0), netManager(0), optionsModel(0)
{
    ...
    // on Mac: sent when you click triponecash: links
...
//
// OSX-specific way of handling triponecash: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in triponecash: URIs
    ...
    connect(netManager, SIGNAL(sslErrors(QNetworkReply *, const QList<QSslError> &)), this,
...
bool PaymentServer::handleURI(const QString &scheme, const QString &s)
{
    ...
    if (GUIUtil::parseTriponeURI(scheme, s, &recipient))
    {
    ...
    else
    {
        Q_EMIT message(tr("URI handling"), tr("URI cannot be parsed! This can be caused by an invalid "
                                              "Tripone address or malformed URI parameters."),
            CClientUIInterface::ICON_WARNING);
    }
...
void PaymentServer::handleURIOrFile(const QString &s)
{
    ...
    // triponecash: CashAddr URI
    QString schemeCash = GUIUtil::triponeURIScheme(Params(), true);
    if (handleURI(schemeCash, s))
    {
        return;
    }

    // triponecash: Legacy URI
    QString schemeLegacy = GUIUtil::triponeURIScheme(Params(), false);
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus &request, SendCoinsRecipient &recipient)
{
    ...
    if (request.IsInitialized())
    {
        ...
        {
            Q_EMIT message(
                tr("Payment request rejected"), tr("Payment request expired."), CClientUIInterface::MSG_ERROR);
    ...
    {
        Q_EMIT message(
            tr("Payment request error"), tr("Payment request is not initialized."), CClientUIInterface::MSG_ERROR);
    ...
    Q_FOREACH (const PAIRTYPE(CScript, CAmount) & sendingTo, sendingTos)
    {
        ...
        {
            // Unauthenticated payment requests to custom tripone addresses are
        ...
        // Tripone amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
        // but CAmount is defined as int64_t. Because of that we need to verify that amounts are in a valid range
        // and no overflow has happened.
        if (!verifyAmount(sendingTo.second))
        {
            Q_EMIT message(
                tr("Payment request rejected"), tr("Invalid payment request."), CClientUIInterface::MSG_ERROR);
            return false;
        }
        ...
        if (txOut.IsDust())
        {
            Q_EMIT message(tr("Payment request error"),
                tr("Requested payment amount of %1 is too small (considered dust).")
                    .arg(TriponeUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
...
void PaymentServer::netRequestFinished(QNetworkReply *reply)
{
    ...
    {
        QString msg =
            tr("Error communicating with %1: %2").arg(reply->request().url().toString()).arg(reply->errorString());
    ...
    if (requestType == BIP70_MESSAGE_PAYMENTREQUEST)
    {
        ...
        if (!request.parse(data))
        {
            ...
            Q_EMIT message(
                tr("Payment request error"), tr("Payment request cannot be parsed!"), CClientUIInterface::MSG_ERROR);
        }
        else if (processPaymentRequest(request, recipient))
            Q_EMIT receivedPaymentRequest(recipient);
    ...
    else if (requestType == BIP70_MESSAGE_PAYMENTACK)
    {
        ...
        {
            QString msg = tr("Bad response from server %1").arg(reply->request().url().toString());
...
void PaymentServer::handlePaymentACK(const QString &paymentACKMsg)
{
    ...
    Q_EMIT message(
        tr("Payment acknowledged"), paymentACKMsg, CClientUIInterface::ICON_INFORMATION | CClientUIInterface::MODAL);
}
...
bool PaymentServer::verifyExpired(const payments::PaymentDetails &requestDetails)
{
    ...
    if (fVerified)
    {
        const QString requestExpires =
            QString::fromStdString(DateTimeStrFormat("%Y-%m-%d %H:%M:%S", (int64_t)requestDetails.expires()));
        qWarning() << QString("PaymentServer::%1: Payment request expired \"%2\".").arg(__func__).arg(requestExpires);
    }
    return fVerified;
}
