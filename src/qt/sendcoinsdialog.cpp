      ...
      fFeeMinimized(true), platformStyle(_platformStyle)
{
    ...
    ui->groupCustomFee->button((int)std::max(0, std::min(1, settings.value("nCustomFeeRadio").toInt())))
        ->setChecked(true);
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    processSendCoinsReturn(prepareStatus, TriponeUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(),
    ...
    Q_FOREACH (const SendCoinsRecipient &rcp, currentTransaction.getRecipients())
    {
        ...
        else
        {
            ...
            QString amount =
                "<b>" + TriponeUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), rcp.amount);
            amount.append("</b>");
    ...
    if (txFee > 0)
    {
        ...
        questionString.append(TriponeUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), txFee));
    ...
    Q_FOREACH (TriponeUnits::Unit u, TriponeUnits::availableUnits())
    {
        if (u != model->getOptionsModel()->getDisplayUnit())
            alternativeUnits.append(TriponeUnits::formatHtmlWithUnit(u, totalAmount));
    }
    questionString.append(
        tr("Total Amount %1")
            .arg(TriponeUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), totalAmount)));
    ...
    const CAmount &watchImmatureBalance)
{
    ...
    if (model && model->getOptionsModel())
    {
        ui->labelBalance->setText(TriponeUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), balance));
    }
}
...
void SendCoinsDialog::processSendCoinsReturn(const WalletModel::SendCoinsReturn &sendCoinsReturn, const QString &msgArg)
{
    ...
    case WalletModel::TransactionCommitFailed:
        msgParams.first = tr("The transaction was rejected! This might happen if some of the coins in your wallet were "
                             "already spent, such as if you used a copy of wallet.dat and coins were spent in the copy "
    ...
    case WalletModel::AbsurdFee:
        msgParams.first =
            tr("A fee higher than %1 is considered an absurdly high fee.")
                .arg(TriponeUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), maxTxFee.Value()));
        break;
...
void SendCoinsDialog::on_buttonChooseFee_clicked() { minimizeFeeSection(false); }
...
void SendCoinsDialog::updateFeeMinimizedLabel()
{
    ...
    else
    {
        ui->labelFeeMinimized->setText(
            TriponeUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), ui->customFee->value()) +
    ...
    if (model && model->getOptionsModel())
        ui->checkBoxMinimumFee->setText(
            tr("Pay only the required fee of %1")
                .arg(TriponeUnits::formatWithUnit(
                         model->getOptionsModel()->getDisplayUnit(), CWallet::GetRequiredFee(1000)) +
                     "/kB"));
}
...
void SendCoinsDialog::updateSmartFeeLabel()
{
    ...
    {
        ui->labelSmartFee->setText(TriponeUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(),
...
void SendCoinsDialog::coinControlClipboardQuantity() { GUIUtil::setClipboard(ui->labelCoinControlQuantity->text()); }
...
{
    GUIUtil::setClipboard(ui->labelCoinControlAfterFee->text()
...
void SendCoinsDialog::coinControlClipboardPriority() { GUIUtil::setClipboard(ui->labelCoinControlPriority->text()); }
// Coin Control: copy label "Dust" to clipboard
void SendCoinsDialog::coinControlClipboardLowOutput() { GUIUtil::setClipboard(ui->labelCoinControlLowOutput->text()); }
...
void SendCoinsDialog::coinControlChangeEdited(const QString &text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        {
            // Invalid address
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid Tripone address"));
        }
        else
        {
            // Valid address
