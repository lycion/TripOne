...
QString TransactionDesc::toHTML(CWallet *wallet, CWalletTx &wtx, TransactionRecord *rec, int unit, QString labelFreeze)
{
    ...
    if (wtx.IsCoinBase() && nCredit == 0)
    {
        ...
        if (wtx.IsInMainChain())
            strHTML += TriponeUnits::formatHtmlWithUnit(unit, nUnmatured) + " (" +
    ...
    else if (nNet > 0)
    {
        ...
        // Credit
        //
        strHTML += "<b>" + tr("Credit") + ":</b> " + TriponeUnits::formatHtmlWithUnit(unit, nNet) + "<br>";
    }
    else
    {
        ...
        if (fAllFromMe)
        {
            ...
            for (const CTxOut &txout : wtx.vout)
            {
...

                {
                    ...
                    if (labelPublic == "") // hide on public label txout
                    {
                        if (toSelf)
                            strHTML += "<b>" + tr("Credit") + ":</b> " +
                                       TriponeUnits::formatHtmlWithUnit(unit, txout.nValue) + " ";

                        strHTML += "<b>" + tr("Debit") + ":</b> " +
                                   TriponeUnits::formatHtmlWithUnit(unit, -txout.nValue) + "<br>";
                    }
                }
            }
            ...
            if (fAllToMe)
            {
                ...
                strHTML +=
                    "<b>" + tr("Total debit") + ":</b> " + TriponeUnits::formatHtmlWithUnit(unit, -nValue) + "<br>";
                strHTML +=
                    "<b>" + tr("Total credit") + ":</b> " + TriponeUnits::formatHtmlWithUnit(unit, nValue) + "<br>";
            }

            CAmount nTxFee = nDebit - wtx.GetValueOut();
            if (nTxFee > 0)
                strHTML +=
                    "<b>" + tr("Transaction fee") + ":</b> " + TriponeUnits::formatHtmlWithUnit(unit, -nTxFee) + "<br>";
        }
        else
        {
            //
            ...
            for (const CTxIn &txin : wtx.vin)
            {
                ...
                {
                    strHTML += "<b>" + tr("Debit") + ":</b> " +
                               TriponeUnits::formatHtmlWithUnit(unit, -wallet->GetDebit(txin, ISMINE_ALL)) + "<br>";
                }
            }
            for (const CTxOut &txout : wtx.vout)
            {
                if (wallet->IsMine(txout))
                {
                    ...
                    strHTML += "<b>" + tr("Credit") + ":</b> " +
                               TriponeUnits::formatHtmlWithUnit(unit, wallet->GetCredit(txout, ISMINE_ALL)) + "<br>";
                }
            }
        }
    }

    strHTML += "<b>" + tr("Net amount") + ":</b> " + TriponeUnits::formatHtmlWithUnit(unit, nNet, true) + "<br>";
    ...
    // Message from normal triponecash:URI (triponecash:123...?message=example)
            ...
            {
                {
                    ...
                    strHTML = strHTML + " " + tr("Amount") + "=" + TriponeUnits::formatHtmlWithUnit(unit, vout.nValue);
