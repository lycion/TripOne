...
{
    return QCoreApplication::translate("tripone-unlimited", psz).toStdString();
}
    ...
    QTranslator &translator)
{
    ...
    // Load e.g. tripone_de.qm (shortcut "de" needs to be defined in tripone.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. tripone_de_DE.qm (shortcut "de_DE" needs to be defined in tripone.qrc)
...
/** Class encapsulating Tripone startup and shutdown.
 * Allows running startup and shutdown in a different thread from the UI thread.
 */
class TriponeCore : public QObject
{
    Q_OBJECT
public:
    explicit TriponeCore();
...
/** Main Tripone application object */
class TriponeApplication : public QApplication
{
    Q_OBJECT
public:
    explicit TriponeApplication(int &argc, char **argv);
    ~TriponeApplication();
    ...
    /// Get window identifier of QMainWindow (TriponeGUI)
...
TriponeCore::TriponeCore() : QObject() {}
void TriponeCore::handleRunawayException(const std::exception *e)
{
    PrintExceptionContinue(e, "Runaway exception");
    Q_EMIT runawayException(QString::fromStdString(strMiscWarning));
}

void TriponeCore::initialize(Config *cfg)
{
    Config &config(*cfg);
    try
    {
...
TriponeApplication::TriponeApplication(int &argc, char **argv)
...
TriponeApplication::~TriponeApplication()
{
    if (coreThread)
    {
...
#ifdef ENABLE_WALLET
void TriponeApplication::createPaymentServer() { paymentServer = new PaymentServer(this); }
#endif

void TriponeApplication::createPlatformStyle()
{
...
void TriponeApplication::createOptionsModel(bool resetSettings)
{
    optionsModel = new OptionsModel(NULL, resetSettings);
    unlimitedModel = new UnlimitedModel(); // BU
}

void TriponeApplication::createWindow(const Config *config, const NetworkStyle *networkStyle)
{
    window = new TriponeGUI(config, platformStyle, networkStyle, 0);
...
void TriponeApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
...
void TriponeApplication::startThread()
{
    if (coreThread)
        return;
    coreThread = new QThread(this);
    TriponeCore *executor = new TriponeCore();
...
void TriponeApplication::parameterSetup()
{
    InitLogging();
    InitParameterInteraction();
}

void TriponeApplication::requestInitialize(Config &config)
{
...
void TriponeApplication::requestShutdown()
{
...
void TriponeApplication::initializeResult(int retval)
{
    ...
    if (retval)
    {
        ...
        if (pwalletMain)
        {
            ...
            window->addWallet(TriponeGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(TriponeGUI::DEFAULT_WALLET);
        ...
        // triponecash: URIs or payment requests:
...
void TriponeApplication::shutdownResult(int retval)
{
    qDebug() << __func__ << ": Shutdown result: " << retval;
    quit(); // Exit main loop after shutdown finished
}

void TriponeApplication::handleRunawayException(const QString &message)
{
    QMessageBox::critical(0, "Runaway exception",
        TriponeGUI::tr("A fatal error occurred. Tripone can no longer continue safely and will quit.") +
            QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}

WId TriponeApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(tripone);
    Q_INIT_RESOURCE(tripone_locale);

    TriponeApplication app(argc, argv);
    ...
    AllowedArgs::TriponeQt allowedArgs(&tweaks);
    try
    {
    ...
    catch (const std::exception &e)
    {
        QMessageBox::critical(
            0, QObject::tr("Tripone"), QObject::tr("Error: Cannot parse program options: %1.").arg(e.what()));
        return EXIT_FAILURE;
    }
    ...
    // For BUCash, first try to migrate from TONE BU settings
    ...
    // This is necessary in case the user wants to run side-by-side TONE chain and BCH chain nodes
    ...
    /// 7. Determine availability of data directory and parse tripone.conf
    ...
    // triponecash: links repeatedly have their payment requests routed to this
    // process:
