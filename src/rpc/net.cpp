...
UniValue getpeerinfo(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "    \"lastsend\": ttt,           (numeric) The time in seconds since epoch (Jan 1 1970 GMT) of the last "
            "send\n"
            "    \"lastrecv\": ttt,           (numeric) The time in seconds since epoch (Jan 1 1970 GMT) of the last "
            "receive\n"
...
UniValue addnode(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 2 || (strCommand != "onetry" && strCommand != "add" && strCommand != "remove"))
        throw runtime_error("addnode \"node\" \"add|remove|onetry\"\n"
                            ...
                            "\nExamples:\n" +
                            HelpExampleCli("addnode", "\"192.168.0.6:9773\" \"onetry\"") +
                            HelpExampleRpc("addnode", "\"192.168.0.6:9773\", \"onetry\""));
...
UniValue disconnectnode(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error("disconnectnode \"node\" \n"
                            ...
                            "1. \"node\"     (string, required) The node (see getpeerinfo for nodes)\n"
                            "\nExamples:\n" +
                            HelpExampleCli("disconnectnode", "\"192.168.0.6:9773\"") +
                            HelpExampleRpc("disconnectnode", "\"192.168.0.6:9773\""));
...
UniValue getaddednodeinfo(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "    \"addresses\" : [\n"
            "       {\n"
            "         \"address\" : \"192.168.0.201:9773\",  (string) The tripone server host and port\n"
...
UniValue getnetworkinfo(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"subversion\": \"/TripOne:x.x.x/\",     (string) the server subversion string\n"
