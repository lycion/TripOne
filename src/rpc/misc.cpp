...
UniValue getinfo(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total tripone balance of the wallet\n"
            ...
            CURRENCY_UNIT +
            "/kB\n"
            "  \"fork\": \"...\"             (string) \"Tripone Cash\" or \"Tripone\".  Will display as Tripone "
            "pre-fork.\n"
    ...
    obj.push_back(Pair("fork", "Tripone Cash"));
...
UniValue validateaddress(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"triponeaddress\"\n"
            "\nReturn information about the given tripone address.\n"
            "\nArguments:\n"
            "1. \"triponeaddress\"     (string, required) The tripone address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only "
            "property returned.\n"
            "  \"address\" : \"triponeaddress\", (string) The tripone address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Tripone address and we have full public key:
...
UniValue createmultisig(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg =
            ...
            "2. \"keys\"       (string, required) A json array of keys which are tripone addresses or hex-encoded "
            "public keys\n"
            "     [\n"
            "       \"key\"    (string) tripone address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"triponeaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"triponeaddress\"  (string, required) The tripone address to use for the signature.\n"
