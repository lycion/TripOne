...
UniValue generatetoaddress(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error("generatetoaddress numblocks address (maxtries)\n"
                            ...
                            "\nArguments:\n"
                            "1. numblocks    (numeric, required) How many blocks are generated immediately.\n"
                            "2. address    (string, required) The address to send the newly generated tripone to.\n"
...
UniValue mkblocktemplate(const UniValue &params, CBlock *pblockOut)
{
    ...
    {
        if (vNodes.empty())
            throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Tripone is not connected!");

        if (IsInitialBlockDownload())
            throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Tripone is downloading blocks...");
    }
...
UniValue getblocktemplate(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "    https://github.com/tripone/bips/blob/master/bip-0022.mediawiki\n"
            "    https://github.com/tripone/bips/blob/master/bip-0009.mediawiki#getblocktemplate_changes\n"
