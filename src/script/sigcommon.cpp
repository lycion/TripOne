    ...
    size_t *nHashedOut)
{
    if (nIn >= txTo.vin.size())
    {
        ...
        // See also: https://lists.linuxfoundation.org/pipermail/tripone-dev/2014-November/006878.html
...
static uint256 SignatureHashTriponeCash(const CScript &scriptCode,
    const CTransaction &txTo,
    unsigned int nIn,
    uint32_t nHashType,
    const CAmount &amount,
    ...
    size_t *nHashedOut)
{
    ...
    {
        return SignatureHashTriponeCash(scriptCode, txTo, nIn, nHashType, amount, nHashedOut);
    }
