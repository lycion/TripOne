...
BOOST_AUTO_TEST_CASE(addrman_simple)
{
    ...
    CService addr1 = CService("250.1.1.1", 9773);
    addrman.Add(CAddress(addr1), source);
    BOOST_CHECK(addrman.size() == 1);
    CAddrInfo addr_ret1 = addrman.Select();
    BOOST_CHECK(addr_ret1.ToString() == "250.1.1.1:9773");

    // Test 3: Does IP address deduplication work correctly.
    //  Expected dup IP should not be added.
    CService addr1_dup = CService("250.1.1.1", 9773);
    ...
    //  have two addrs.
    CService addr2 = CService("250.1.1.2", 9773);
...
BOOST_AUTO_TEST_CASE(addrman_ports)
{
    ...
    BOOST_CHECK(addr_ret2.ToString() == "250.1.1.1:9773");
    ...
    BOOST_CHECK(addr_ret3.ToString() == "250.1.1.1:9773");
}
...
BOOST_AUTO_TEST_CASE(addrman_select)
{
    ...
    CService addr2 = CService("250.3.1.1", 9773);
    CService addr3 = CService("250.3.2.2", 9999);
    CService addr4 = CService("250.3.3.3", 9999);

    addrman.Add(CAddress(addr2), CService("250.3.1.1", 9773));
    addrman.Add(CAddress(addr3), CService("250.3.1.1", 9773));
    addrman.Add(CAddress(addr4), CService("250.4.1.1", 9773));

    // Add three addresses to tried table.
    CService addr5 = CService("250.4.4.4", 9773);
    CService addr6 = CService("250.4.5.5", 7777);
    CService addr7 = CService("250.4.6.6", 9773);

    addrman.Add(CAddress(addr5), CService("250.3.1.1", 9773));
    addrman.Good(CAddress(addr5));
    addrman.Add(CAddress(addr6), CService("250.3.1.1", 9773));
    addrman.Good(CAddress(addr6));
    addrman.Add(CAddress(addr7), CService("250.1.1.3", 9773));
    ...
    BOOST_CHECK(addrman.Select().ToString() == "250.4.6.6:9773");
    BOOST_CHECK(addrman.Select().ToString() == "250.3.2.2:9999");
    BOOST_CHECK(addrman.Select().ToString() == "250.3.3.3:9999");
    BOOST_CHECK(addrman.Select().ToString() == "250.4.4.4:9773");
}
...
BOOST_AUTO_TEST_CASE(addrman_find)
{
    ...
    CAddress addr1 = CAddress(CService("250.1.2.1", 9773));
    CAddress addr2 = CAddress(CService("250.1.2.1", 9999));
    CAddress addr3 = CAddress(CService("251.255.2.1", 9773));
    ...
    BOOST_CHECK(info1);
    if (info1)
        BOOST_CHECK(info1->ToString() == "250.1.2.1:9773");
    ...
    BOOST_CHECK(info3);
    if (info3)
        BOOST_CHECK(info3->ToString() == "251.255.2.1:9773");
}
...
BOOST_AUTO_TEST_CASE(addrman_create)
{
    ...
    BOOST_CHECK(pinfo->ToString() == "250.1.2.1:9773");

    CAddrInfo *info2 = addrman.Find(addr1);
    BOOST_CHECK(info2->ToString() == "250.1.2.1:9773");
}
...
BOOST_AUTO_TEST_CASE(addrman_getaddr)
{
    ...
    CAddress addr1 = CAddress(CService("250.250.2.1", 9773));
    addr1.nTime = GetAdjustedTime(); // Set time so isTerrible = false
    CAddress addr2 = CAddress(CService("250.251.2.2", 9999));
    addr2.nTime = GetAdjustedTime();
    CAddress addr3 = CAddress(CService("251.252.2.3", 9773));
    addr3.nTime = GetAdjustedTime();
    CAddress addr4 = CAddress(CService("252.253.3.4", 9773));
    addr4.nTime = GetAdjustedTime();
    CAddress addr5 = CAddress(CService("252.254.4.5", 9773));
...
BOOST_AUTO_TEST_CASE(caddrinfo_get_tried_bucket)
{
    ...
    CAddress addr1 = CAddress(CService("250.1.1.1", 9773));
    ...
    {
        CAddrInfo infoi =
            CAddrInfo(CAddress(CService("250.1.1." + std::to_string(i))), CNetAddr("250.1.1." + std::to_string(i)));
    ...
    for (int j = 0; j < 255; j++)
    {
        CAddrInfo infoj = CAddrInfo(
            CAddress(CService("250." + std::to_string(j) + ".1.1")), CNetAddr("250." + std::to_string(j) + ".1.1"));
...
BOOST_AUTO_TEST_CASE(caddrinfo_get_new_bucket)
{
    ...
    {
        CAddrInfo infoj =
            CAddrInfo(CAddress(CService(std::to_string(250 + (j / 255)) + "." + std::to_string(j % 256) + ".1.1")),
                CNetAddr("251.4.1.1"));
    ...
    for (int p = 0; p < 255; p++)
    {
        CAddrInfo infoj = CAddrInfo(CAddress(CService("250.1.1.1")), CNetAddr("250." + std::to_string(p) + ".1.1"));
        int bucket = infoj.GetNewBucket(nKey1);
        buckets.insert(bucket);
    }
