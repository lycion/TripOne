...
#define BOOST_TEST_MODULE Tripone Test Suite

#include "test_tripone.h"
...
TestingSetup::TestingSetup(const std::string &chainName) : BasicTestingSetup(chainName)
{
    ...
    ClearDatadirCache();
    pathTemp = GetTempPath() / strprintf("test_tripone_%lu_%i", (unsigned long)GetTime(), (int)(GetRand(1 << 30)));
...
struct StartupShutdown
{
    StartupShutdown()
    {
        ...
        optDef.add_options()("testhelp", "program options information")(
            ...
            "log_tripone", value<std::string>()->required(), "tripone logging destination (console, none)");
        variables_map opts;
        ...
        if (opts.count("log_tripone"))
        {
            std::string s = opts["log_tripone"].as<std::string>();
            if (s == "console")
            {
                /* To enable this, add
                   -- --log_tripone console
                   to the end of the test_tripone argument list. */
