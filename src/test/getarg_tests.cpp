...
static void ResetArgs(const std::string &strArg, Kind kind = TRIPONED)
{
    ...
    vecArg.insert(vecArg.begin(), "testtripone");
    ...
    else if (kind == TRIPONED)
        ParseParameters(vecChar.size(), &vecChar[0], AllowedArgs::Triponed(&tweaks));
    else
        ParseParameters(vecChar.size(), &vecChar[0], AllowedArgs::TriponeCli());
}
...
BOOST_AUTO_TEST_CASE(tweakArgs)
{
    ...
    // Test tripone-cli accepts unknown tweaks
    ResetArgs("-some.tweak=something", TRIPONE_CLI);
