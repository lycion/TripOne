...
BOOST_AUTO_TEST_CASE(base58_EncodeBase58)
{
    UniValue tests = read_json(std::string(
        json_tests::base58_encode_decode, json_tests::base58_encode_decode + sizeof(json_tests::base58_encode_decode)));
    for (unsigned int idx = 0; idx < tests.size(); idx++)
    {
        ...
        BOOST_CHECK_MESSAGE(EncodeBase58(begin_ptr(sourcedata), end_ptr(sourcedata)) == base58string, strTest);
    }
}
...
BOOST_AUTO_TEST_CASE(base58_DecodeBase58)
{
    ...
    for (unsigned int idx = 0; idx < tests.size(); idx++)
    {
        ...
        BOOST_CHECK_MESSAGE(
            result.size() == expected.size() && std::equal(result.begin(), result.end(), expected.begin()), strTest);
    }
...
{
    UniValue tests = read_json(std::string(
        json_tests::base58_keys_valid, json_tests::base58_keys_valid + sizeof(json_tests::base58_keys_valid)));
    std::vector<unsigned char> result;
    CTriponeSecret secret;
    ...
    for (unsigned int idx = 0; idx < tests.size(); idx++)
    {
        ...
        if (isPrivkey)
        {
            ...
            // Note: CTriponeSecret::SetString tests isValid, whereas CTriponeAddress does not!
            BOOST_CHECK_MESSAGE(secret.SetString(exp_base58string), "!SetString:" + strTest);
            BOOST_CHECK_MESSAGE(secret.IsValid(), "!IsValid:" + strTest);
            CKey privkey = secret.GetKey();
            BOOST_CHECK_MESSAGE(privkey.IsCompressed() == isCompressed, "compressed mismatch:" + strTest);
            BOOST_CHECK_MESSAGE(
                privkey.size() == exp_payload.size() && std::equal(privkey.begin(), privkey.end(), exp_payload.begin()),
...
BOOST_AUTO_TEST_CASE(base58_keys_invalid)
{
    // Negative testcases
    UniValue tests = read_json(std::string(
        json_tests::base58_keys_invalid, json_tests::base58_keys_invalid + sizeof(json_tests::base58_keys_invalid)));
    std::vector<unsigned char> result;
    CTriponeSecret secret;
    CTxDestination destination;
