...
// BU add lockstack stuff here for tripone-miner, because I need to carefully
...
class TriponeMinerArgs : public AllowedArgs::TriponeCli
{
public:
    TriponeMinerArgs(CTweakMap *pTweaks = nullptr)
    {
...
static UniValue CpuMineBlock(unsigned int searchDuration, const UniValue &params, bool &found)
{
    ...
    while ((GetTime() < start + searchDuration) && !found)
    {
        ...
        // and the block will be rejected.  So do not advance time (let it be advanced by triponed every time we
        // request a new block).
    ...
    while (0 != nblocks)
    {
        ...
        if (strPrint != "")
        {
            ...
            else
            {
                ...
                // The result is sent to triponed above when the loop gets to it.
...
int main(int argc, char *argv[])
{
    ...
    try
    {
        std::string appname("tripone-miner");
        std::string usage = "\n" + _("Usage:") + "\n" + "  " + appname + " [options] " + "\n";
        int ret = AppInitRPC(usage, TriponeMinerArgs(), argc, argv);
