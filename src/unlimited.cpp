...
UniValue expedited(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() < 2)
        throw runtime_error("expedited block|tx \"node IP addr\" on|off\n"
                            ...
                            "\nExamples:\n" +
                            HelpExampleCli("expedited", "block \"192.168.0.6:9773\" on") +
                            HelpExampleRpc("expedited", "\"block\", \"192.168.0.6:9773\", \"on\""));
...
UniValue pushtx(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error("pushtx \"node\"\n"
                            ...
                            "\nExamples:\n" +
                            HelpExampleCli("pushtx", "\"192.168.0.6:9773\" ") +
                            HelpExampleRpc("pushtx", "\"192.168.0.6:9773\", "));
...
void UnlimitedSetup(void)
{
    ...
    GenerateTripones(GetBoolArg("-gen", DEFAULT_GENERATE), GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), Params());
}
...
static bool ProcessBlockFound(const CBlock *pblock, const CChainParams &chainparams)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("TriponeMiner: generated block is stale");
    }
...

    {
        ...
        if (!ProcessNewBlock(state, chainparams, nullptr, pblock, true, nullptr, false))
            return error("TriponeMiner: ProcessNewBlock, block not accepted");
    }
...
void static TriponeMiner(const CChainParams &chainparams)
{
    LOGA("TriponeMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tripone-miner");
    ...
    try
    {
        ...
        while (true)
        {
            ...
            {
                LOGA("Error in TriponeMiner: Keypool ran out, please call keypoolrefill before restarting the "
                     "mining thread\n");
                return;
            }
            ...
            LOGA("Running TriponeMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
                pblock->GetBlockSize());
    ...
    {
        LOGA("TriponeMiner terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LOGA("TriponeMiner runtime error: %s\n", e.what());
        return;
    }
}

void GenerateTripones(bool fGenerate, int nThreads, const CChainParams &chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TriponeMiner, boost::cref(chainparams)));
}
...
UniValue getgenerate(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error("getgenerate\n"
                            ...
                            std::string(TRIPONE_CONF_FILENAME) +
                            " setting gen)\n"
...
UniValue setgenerate(const UniValue &params, bool fHelp)
{
    ...
    GenerateTripones(fGenerate, nGenProcLimit, Params());
...
extern UniValue getminercomment(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error("getminercomment\n"
                            ...
                            "after the Tripone Unlimited parameters."
                            "\nResult\n"
...
extern UniValue setminercomment(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error("setminercomment\n"
                            ...
                            "the Tripone Unlimited parameters.\n Comments that are too long will be truncated."
                            "\nExamples:\n" +
                            HelpExampleCli("setminercomment", "\"tripone is fundamentally emergent consensus\"") +
                            HelpExampleRpc("setminercomment", "\"tripone is fundamentally emergent consensus\""));
...
UniValue validateblocktemplate(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw runtime_error(
            ...
            "See https://en.tripone.it/wiki/BIP_0022 for full specification.\n"
...
UniValue getaddressforms(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw runtime_error("getaddressforms \"address\"\n"
                            ...
                            "\"triponecash\": \"triponecash prefixed address\",\n"
    ...
    node.push_back(Pair("triponecash", cashAddr));
