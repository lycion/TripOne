...
UniValue importprivkey(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey \"triponeprivkey\" ( \"label\" rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"triponeprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
UniValue importprivatekeys(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1)
        throw runtime_error(
            "importprivatekeys [rescan | no-rescan] \"triponeprivatekey\"...\n"
            ...
            "2. \"triponeprivatekey\"   (string, at least 1 required) The private keys (see dumpprivkey)\n"
...
UniValue importaddress(const UniValue &params, bool fHelp)
{
    ...
    {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Tripone address or script");
    }
...
UniValue dumpprivkey(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error("dumpprivkey \"triponeaddress\"\n"
                            "\nReveals the private key corresponding to 'triponeaddress'.\n"
                            "Then the importprivkey can be used with this output\n"
                            "\nArguments:\n"
                            "1. \"triponeaddress\"   (string, required) The tripone address for the private key\n"
                            "\nResult:\n"
    ...
    return CTriponeSecret(vchSecret).ToString();
}
...
UniValue dumpwallet(const UniValue &params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by Tripone %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
    ...
    for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++)
    {
        ...
        {
            file << strprintf("%s %s ", CTriponeSecret(key).ToString(), strTime);
