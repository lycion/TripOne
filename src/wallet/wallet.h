...
#ifndef TRIPONE_WALLET_WALLET_H
#define TRIPONE_WALLET_WALLET_H
...
//! We can allow a maximum sized free transaction in the Tripone Cash Network.
...
public:
    ...
    inline void SerializationOp(Stream &s, Operation ser_action)
    {
        ...
        if (ser_action.ForRead())
        {
            ...
            {
                CDataStream ss(
                    std::vector<char>(strComment.begin() + nSepPos + 1, strComment.end()), s.GetType(), s.GetVersion());
                ss >> mapValue;
