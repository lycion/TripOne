...
UniValue getnewaddress(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error("getnewaddress ( \"account\" )\n"
                            "\nReturns a new Tripone address for receiving payments.\n"
                            ...
                            "\nResult:\n"
                            "\"triponeaddress\"    (string) The new tripone address\n"
                            "\nExamples:\n" +
...
UniValue getaccountaddress(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nDEPRECATED. Returns the current Tripone address for receiving payments to this account.\n"
            "\nArguments:\n"
            ...
            "\nResult:\n"
            "\"triponeaddress\"   (string) The account tripone address\n"
            "\nExamples:\n" +
...
UniValue getrawchangeaddress(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error("getrawchangeaddress\n"
                            "\nReturns a new Tripone address, for receiving change.\n"
...
UniValue setaccount(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount \"triponeaddress\" \"account\"\n"
            "\nDEPRECATED. Sets the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"triponeaddress\"  (string, required) The tripone address to be associated with an account.\n"
    ...
    {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Tripone address");
    }
...
UniValue getaccount(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error("getaccount \"triponeaddress\"\n"
                            "\nDEPRECATED. Returns the account associated with the given address.\n"
                            "\nArguments:\n"
                            "1. \"triponeaddress\"  (string, required) The tripone address for account lookup.\n"
                            "\nResult:\n"
...
UniValue getaddressesbyaccount(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error("getaddressesbyaccount \"account\"\n"
                            ...
                            "  \"triponeaddress\"  (string) a tripone address associated with the given account\n"
                            "  ,...\n"
                            "]\n"
                            "\nExamples:\n" +
...
UniValue sendtoaddress(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "sendtoaddress \"triponeaddress\" amount ( \"comment\" \"comment-to\" subtractfeefromamount )\n"
            "\nSend an amount to a given address.\n" +
            HelpRequiringPassphrase() + "\nArguments:\n"
                                        "1. \"triponeaddress\"  (string, required) The tripone address to send to.\n"
            ...
            "being sent.\n"
            "                             The recipient will receive less tripones than you enter in the amount "
            "field.\n"
            "\nResult:\n"
...
UniValue listaddressgroupings(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp)
        throw runtime_error("listaddressgroupings\n"
                            ...
                            "  [\n"
                            "    [\n"
                            "      \"triponeaddress\",     (string) The tripone address\n"
...
UniValue signmessage(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage \"triponeaddress\" \"message\"\n"
            "\nSign a message with the private key of an address" +
            HelpRequiringPassphrase() +
            "\n"
            "\nArguments:\n"
            "1. \"triponeaddress\"  (string, required) The tripone address to use for the private key.\n"
...
UniValue getreceivedbyaddress(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error("getreceivedbyaddress \"triponeaddress\" ( minconf )\n"
                            "\nReturns the total amount received by the given triponeaddress in transactions with at "
                            "least minconf confirmations.\n"
                            "\nArguments:\n"
                            "1. \"triponeaddress\"  (string, required) The tripone address for transactions.\n"
...
UniValue sendfrom(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom \"fromaccount\" \"totriponeaddress\" amount ( minconf \"comment\" \"comment-to\" )\n"
            "\nDEPRECATED (use sendtoaddress). Sent an amount from an account to a tripone address." +
            HelpRequiringPassphrase() +
            "\n"
            "\nArguments:\n"
            "1. \"fromaccount\"       (string, required) The name of the account to send funds from. May be the "
            "default account using \"\".\n"
            "2. \"totriponeaddress\"  (string, required) The tripone address to send funds to.\n"
...
UniValue sendmany(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric or string) The tripone address is the key, the numeric amount (can be "
            "string) in " +
            CURRENCY_UNIT +
            " is the value\n"
            "      ,...\n"
            ...
            "                           Those recipients will receive less tripones than you enter in their "
    ...
    for (const std::string &name_ : keys)
    {
        ...
        if (!IsValidDestination(dest))
        {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Tripone address: ") + name_);
        }
...
UniValue addmultisigaddress(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg =
            ...
            "Each key is a Tripone address or hex-encoded public key.\n"
            "If 'account' is specified (DEPRECATED), assign address to that account.\n"

            "\nArguments:\n"
            "1. nrequired        (numeric, required) The number of required signatures out of the n keys or "
            "addresses.\n"
            "2. \"keysobject\"   (string, required) A json array of tripone addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) tripone address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"account\"      (string, optional) DEPRECATED. An account to assign the addresses to.\n"

            "\nResult:\n"
            "\"triponeaddress\"  (string) A tripone address associated with the keys.\n"
...
UniValue listtransactions(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 4)
        throw runtime_error(
            ...
            "    \"address\":\"triponeaddress\",    (string) The tripone address of the transaction. Not present for \n"
...
UniValue gettransaction(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "      \"address\" : \"triponeaddress\",   (string) The tripone address involved in the transaction\n"
...
UniValue walletpassphrase(const UniValue &params, bool fHelp)
{
    ...
    if (pwalletMain->IsCrypted() && (fHelp || params.size() != 2))
        throw runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending tripones\n"
            "\nArguments:\n"
...
UniValue encryptwallet(const UniValue &params, bool fHelp)
{
    ...
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending tripone\n" +
            HelpExampleCli("walletpassphrase", "\"my pass phrase\"") + "\nNow we can so something like sign\n" +
            HelpExampleCli("signmessage", "\"triponeaddress\" \"test message\"") +
    ...
    StartShutdown();
    return "wallet encrypted; Tripone server stopping, restart to run with encrypted wallet. The keypool has been "
...
UniValue lockunspent(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending tripones.\n"
...
UniValue listunspent(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "3. \"addresses\"    (string) A json array of tripone addresses to filter\n"
            "    [\n"
            "      \"address\"   (string) tripone address\n"
            "      ,...\n"
            "    ]\n"
            "\nResult\n"
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"address\" : \"address\",  (string) the tripone address\n"
    ...
    if (params.size() > 2)
    {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++)
        {
            ...
            if (!IsValidDestination(address))
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Tripone address: ") + input.get_str());
