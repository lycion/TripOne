...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Tripone users should be well used to
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Tripone " + FormatFullVersion();
...
#if 0 // Disabled until a Tripone Cash compatible "bitnodes" site becomes available
...
void ThreadAddressSeeding()
{
    ...
    {
        // TODO: re-enable bitnodes seeding once a site is available for the TriponeCash chain.
        // BitnodesAddressSeed();
    ...
    if (mapArgs.count("-connect") && mapMultiArgs["-connect"].size() > 0)
    {
        for (int64_t nLoop = 0;; nLoop++)
        {
            ...
            for (const std::string &strAddr : mapMultiArgs["-connect"])
            {
                ...
                //      tripone.conf/commandline args as "-connect", we don't use the semaphore to limit outbound
                //      connections
...
void StartNode(boost::thread_group &threadGroup, CScheduler &scheduler)
{
    ...
    // connections based on the configured "addnode" options from tripone.conf/command line, however the old
