 ...
 *
 //www.tripone.org/),
 * This is the developer documentation of the reference client for an experimental new digital currency called Tripone
 (https:
 * which enables instant payments to anyone, anywhere in the world. Tripone uses peer-to-peer technology to operate
...
bool AppInit(int argc, char *argv[])
{
    ...
    // Parameters
    //
    // If Qt is used, parameters/tripone.conf are parsed in qt/tripone.cpp's main()
    AllowedArgs::Triponed allowedArgs(&tweaks);
    try
    {
    ...
    try
    {
        ...
        {
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "tripone:") &&
                !boost::algorithm::istarts_with(argv[i], "triponecash:"))
            {
                fCommandLine = true;
                break;
            }
        ...
        if (fCommandLine)
        {
            fprintf(stderr, "Error: There is no RPC client functionality in triponed anymore. Use the tripone-cli "
                            "utility instead.\n");
            return false;
        }
#ifndef WIN32
        ...
        if (fDaemon)
        {
            fprintf(stdout, "Tripone server starting\n");
...
int main(int argc, char *argv[])
{
    ...
    // Connect triponed signal handlers
    noui_connect();
