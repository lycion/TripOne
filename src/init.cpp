...
#endif
    GenerateTripones(false, 0, Params());
    StopNode();
    StopTorControl();
...
/** Sanity checks
 *  Ensure that Tripone is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // UAHF - TriponeCash service bit
    nLocalServices |= NODE_TRIPONE_CASH;
    ...
    // Make sure only a single Tripone process is using the data directory.
    ...
    LOGA("Using config file %s\n", GetConfigFile(GetArg("-conf", TRIPONE_CONF_FILENAME)).string());
...
// ********************************************************* Step 7: load wallet
    ...
    // It is possible that we need to override the old settings in QT and tripone.conf.
