...
const char *const TRIPONE_CONF_FILENAME = "tripone.conf";
const char *const TRIPONE_PID_FILENAME = "triponed.pid";
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\Tripone
// Windows >= Vista: C:\Users\Username\AppData\Roaming\Tripone
// Mac: ~/Library/Application Support/Tripone
// Unix: ~/.tripone
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Tripone";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Tripone";
#else
    // Unix
    return pathRet / ".tripone";
#endif
#endif
}
...
{
    fs::ifstream streamConfig(GetConfigFile(GetArg("-conf", TRIPONE_CONF_FILENAME)));
    if (!streamConfig.good())
        return; // No tripone.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override tripone.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", TRIPONE_PID_FILENAME));
